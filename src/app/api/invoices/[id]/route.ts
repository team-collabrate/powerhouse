...
export async function PATCH(request: Request, { params }: Params) {
  ...
  // once it's left draft, amount and issue date are locked
  if (existing.status !== "draft" && (input.amount !== undefined || input.issueDate !== undefined)) {
    return fail(
      "INVALID_STATE",
      "Amount and issue date can only change while the invoice is a draft",
      409,
    );
  }
